...
export function getTagList(
  ...
  return components.map((component: Component) => {
    ...
    const symbol = component.tagName ? sourceSymbols.get(component.tagName) : undefined;
...
export function getComponentsExportsMap(
  ...
  return new Map(
    customElementsManifest.modules?.map(
      (mod) => mod.exports
        ?.filter(e => e.kind === 'custom-element-definition' && e.declaration.module)
        .map(e => [
          e.name,
          {
            symbol: e.declaration.name,
            module: options.modulePathTemplate ? options.modulePathTemplate(e.declaration.name, e.declaration.module!) : e.declaration.module,
